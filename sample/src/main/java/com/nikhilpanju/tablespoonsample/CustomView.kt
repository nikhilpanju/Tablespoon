...
class CustomView @JvmOverloads constructor(
    ...
    @delegate:BooleanAttr(R2.styleable.CustomView_bool_attr)
    var someBool: Boolean by dynamicBooleanAttr(false)

    @delegate:ColorAttr(R2.styleable.CustomView_color_state_attr)
    var colorStateListAttr: ColorStateList? by dynamicColorStateAttr(null)

    @delegate:ColorAttr(R2.styleable.CustomView_color_attr)
    var colorAttr: Int by dynamicIntAttr(0xFFF1BB5A.toInt())

    @delegate:DimensionAttr(R2.styleable.CustomView_dimension_int_attr)
    var dimIntAttr: Int by dynamicIntAttr(0)

    @DimensionAttr(R2.styleable.CustomView_dimension_float_attr)
    var dimFloatAttr: Float = 10f

    @delegate:DrawableAttr(R2.styleable.CustomView_drawable_attr)
    var drawableAttr: Drawable? by dynamicDrawableAttr(null)

    @delegate:FloatAttr(R2.styleable.CustomView_float_attr)
    var floatAttr: Float by dynamicFloatAttr(4f)

    @delegate:IntAttr(R2.styleable.CustomView_integer_attr)
    var intAttr: Int by dynamicIntAttr(66)

    @delegate:IntAttr(R2.styleable.CustomView_enum_attr)
    var enumAttr: Int by dynamicIntAttr(0)

    @delegate:IntAttr(R2.styleable.CustomView_flags_attr)
    var flagAttr: Int by dynamicIntAttr(0)

    @delegate:ResourceIdAttr(R2.styleable.CustomView_reference_attr)
    ...
    init {
        TableSpoon.init(this, attrs, R.styleable.CustomView, defStyleAttr, defStyleRes) {
            stringAttr = getString(R.styleable.CustomView_string_attr) ?: stringAttr
        }
